...
impl<'ctx> PLIFloatDecimalValue<'ctx> {
    ...
    pub fn set_precision<'a>(&self, value: IntValue<'ctx>, compiler: &Compiler<'a, 'ctx>) {
        ...
        unsafe {
            let intval_zero = compiler.context.i8_type().const_zero();
            let intval_one = compiler.context.i8_type().const_int(1, false);
            let precision_ptr = compiler
                .builder
                .build_gep(
                    malloc_ptr,
                    &[intval_zero, intval_one],
                    "gepping_to_precision",
                )
                .unwrap();

            compiler.builder.build_store(precision_ptr, value).unwrap();
        }
...
impl<'a, 'ctx> Puttable<'a, 'ctx> for PLIFloatDecimalValue<'ctx> {
    unsafe fn print_object(&self, compiler: &'a Compiler<'a, 'ctx>) {
        ...
        unsafe {
            ...
            let precision_ptr = compiler
                .builder
                .build_gep(malloc_ptr, &[intval_zero, intval_zero], "gepping_to_float")
                .unwrap();
...
impl<'a, 'ctx> Mathable<'a, 'ctx> for PLIFloatDecimalValue<'ctx> {
    fn convert_to_float(&self, compiler: &'a Compiler<'a, 'ctx>) -> FloatValue<'ctx> {
        ...
        let malloc_ptr = compiler.builder.build_free(malloc_ptr);
    ...
    for PLIFloatDecimalValue<'ctx>
{
    unsafe fn create_mathable(
        ...
        values.push(float.as_basic_value_enum());
        values.push(
            compiler
                .context
                .i8_type()
