...
use crate::{codegen::{codegen::CodeGenable, utils::{get_current_function, self}}, ast};
  ...
  unsafe fn codegen_with_error_info(self, compiler: &'a crate::codegen::codegen::Compiler<'a, 'ctx>)
                -> Result<Box<dyn inkwell::values::AnyValue<'ctx> + 'ctx>, Box<dyn Error>> {
                  ...
                  else
                  {
                            ...
                            if let Some(block) = x
                            {
                                debug!("Does function have terminator {:?}", compiler.builder.get_insert_block().unwrap().get_terminator());
                            ...
                            else
                            {
                                ...
                                let mut placeholder_block = compiler.context.append_basic_block(get_current_function(compiler), "PLACEHOLDER");

                                compiler.function_properties.borrow_mut().store_placeholder_block(&self.label_to_go_to, placeholder_block);
